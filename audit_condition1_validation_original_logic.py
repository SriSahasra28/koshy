...
USING ORIGINAL CROSSOVER-BASED PSAR SIGNAL LOGIC

WHAT THIS SCRIPT DOES:
1. Takes a symbol and condition ID as input
2. Fetches actual alerts for that symbol/condition from database
3. For EACH alert:
   a. Fetches OHLC data from MySQL (resampled to required timeframe)
   b. Calculates indicators (PSAR, Stochastic K/D, Heikin-Ashi candles)
   c. Calculates PSAR signals using CROSSOVER logic (matches original code)
   d. Validates Condition 1 requirements:
      - K value in range [kline_start, kline_end]
      - PSAR signal matches signaldirection (using crossover-based signals)
      - Candle type matches (candle1)
4. Reports which alerts are VALID (correctly triggered) vs INVALID (shouldn't have triggered)

KEY DIFFERENCE FROM audit_condition1_validation.py:
- Uses get_psar_signals() (crossover-based) instead of get_psar_signal() (position-based)
- Signal = 1 only when price crosses ABOVE PSAR
- Signal = -1 only when price crosses BELOW PSAR
- Signal = 0 otherwise (no crossover)

Usage: python audit_condition1_validation_original_logic.py <symbol> <condition_id> [max_alerts]
Example: python audit_condition1_validation_original_logic.py ASHOKLEY26JANFUT 6 10
"""
import pymysql
import pandas as pd
import numpy as np
...
def heikin_ashi_numpy(open_, high, low, close):
    """Calculate Heikin-Ashi candles using numpy"""
...
def get_psar_signals(close, psar_values):
    """
    Calculate PSAR signals using CROSSOVER logic (matches original code exactly)
    Signal = 1 when price crosses ABOVE PSAR
    Signal = -1 when price crosses BELOW PSAR
    Signal = 0 otherwise (no crossover)
    """
...
def validate_alert(alert_row, condition_details, redis_client=None):
    """Validate a single alert against Condition 1 requirements (uses ORIGINAL crossover-based PSAR signal logic)"""
    ...
    if ohlc_df.empty:
        return {
            'valid': False,
            'reason': "No OHLC data found in MySQL"
        }
    ...
    # CRITICAL: Calculate PSAR signals using CROSSOVER logic (matches original code)
    close_prices = ohlc_df['close'].values.astype(float)
    psar_signals = get_psar_signals(close_prices, psar_data)
    psar_signal = psar_signals[candle_idx]  # Get signal for this specific candle
    ...
    if psar_signal != signaldirection:
        validation_results.append(f"PSAR signal {psar_signal} != required {signaldirection} (crossover-based)")
    ...
    if len(validation_results) == 0:
        return {
            ...
            'details': {
                ...
                'psar_signal': psar_signal,  # Crossover-based signal (can be 0, 1, or -1)
                'ha_open': ha_open_val,
                'ha_high': ha_high_val,
                'ha_low': ha_low_val,
...
def validate_alerts_batch(actual_alerts_df, condition_details, redis_client=None, max_alerts=10):
    """Validate a batch of alerts - uses MySQL for OHLC data and ORIGINAL crossover-based PSAR signals"""
    results = []
    skipped = []
    ...
    print(f"\nValidating {len(alerts_to_validate)} alerts (using ORIGINAL crossover-based PSAR signal logic)...")
...
def print_validation_report(validation_results, skipped_alerts=None):
    ...
    print("\n" + "="*80)
    print("VALIDATION REPORT (ORIGINAL CROSSOVER-BASED PSAR SIGNAL LOGIC)")
    print("="*80)
    ...
    if skipped_count > 0:
        print(f"  Skipped (no MySQL data): {skipped_count}")
    ...
    if invalid_alerts:
        for alert in invalid_alerts:
            ...
            if alert['details']:
                psar_signal = alert['details'].get('psar_signal', 'N/A')
                print(f"  K={alert['details'].get('stoch_k', 'N/A'):.2f}, "
                      f"PSAR_signal={psar_signal} (crossover-based), "
    ...
    if skipped_alerts and len(skipped_alerts) > 0:
        print("\n" + "-"*80)
        print("SKIPPED ALERTS (No MySQL Data Available):")
        print("-"*80)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print("Usage: python audit_condition1_validation_original_logic.py <symbol> <condition_id> [max_alerts]")
        print("Example: python audit_condition1_validation_original_logic.py ASHOKLEY26JANFUT 6 10")
        sys.exit(1)
    ...
    # Validate alerts (using MySQL for OHLC data and ORIGINAL crossover-based PSAR signals)
    print("\nUsing MySQL for OHLC data and ORIGINAL crossover-based PSAR signal logic...")
    validation_results, skipped_alerts = validate_alerts_batch(actual_alerts_df, condition_details, max_alerts=max_alerts)
